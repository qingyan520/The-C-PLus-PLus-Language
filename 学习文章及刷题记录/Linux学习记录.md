...
用户态：系统调用，时间片到了导致信号切换，异常中断，陷阱进入内核态

内核太：系统调用返回，进程切换完毕，异常，中断陷阱处理后进入用户态
...
命令行上启动多个进程，协同完成某种任务，其中第一个创建出来的进程，称之为组长进程
...
守护进程本质上是孤儿进程，它有自己的使命

创建守护进程：

核心：setsid():不能是进程组的组长(命令行上牵动的多个进程，协同完成某种任务，其中第一个被创建出来的几次呢，称之为组长进程)
...
int main()
{
    ...
    //孙子进程
    chdir("/");//可选的选项
    //dev/null:字符文件，设备。：屏蔽，drop,输入输出信息
   close(0);
...
![image-20211112150554283](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20211112150554283.png)
...
在一个程序里的一个执行路线就叫做线程，更准确地定义是：线程是一个进程内部的控制列

一切进程至少都有一个执行线程

线程在进程内部运行，本质是进程地址空间内运行

在Linux中，在CPU眼中，看到的PCB都要比传统的进程更加轻量化

透过进程虚拟地址空间，可以看到进程的大部分资源，将进程资源合理分配给每个执行流，就形成了执行流



进程：站在内核角度，承担分配系统资源的基本单位，叫做进程

如何理解之前的进程？？所有进程都是只有一个task_struct,该进程内部，只有一个执行流



Linux中，站在CPU的角度，能否识别task_struct是进程？还是线程？

不能，也不需要了，CPU只关心一个一个的独立执行流

在CPU看来，task_struct<=OS原理的的进程控制块



Linux下，并不存在真正意义上的多线程,而是用进程模拟的

Linux中所有的执行流都叫做轻量级进程

如果要支持真的线程，当线程足够多的时候，OS要不要管理线程？要！



Linux没有真正意义上的线程，所以，LInux也没有绝对意义上的线程相关的系统调用

提供创建轻量级进程的接口，创建进程，共享空间

vfork:创建父子共享空间

上面说的，都是站在内核角度，站在用户角度，我就创建线程，原生线程库的方案解决的



基于轻量级进程的系统调用，在用户层模拟实现一套线程接口，pthread



32位地址如果只有一张表，映射表项一共需要2^32

MMU：内存管理单元硬件完成映射，集成在CPU内，



计算密集型：执行流的大部分任务主要以计算为主，加密解密，排序查找

IO密集型：执行流执行的大部分任务是以IO为主的：刷磁盘，访问数据库，访问网络



线程的优点：

创建一个新线程的待见要比创建一个新进程小得多

与进程之间的切换相比，线程之间的切换需要操作系统多的工作要少很多
...
计算密集型的应用，为了能在多处理器系统上运行，将计算分解到多个进程中实现
...
一个很少被外部时间阻塞的计算密集型线程往往无法与其它线程共享与i个处理器，如果计算密集型线程的数量比可用的处理器多，那么可能有较大的性能损失，这里的损失指的是增加了额外的同步与调度开销，而可用的资源不变

健壮性降低

编写多线程程序需要更全面更深入的考虑，在一个多线程程序里，因时间分配上的细微偏差或者因共享了不该共享的变量而造成不良影响的可能性是很大的，换句话说线程缺乏保护
...
单个线程如果出现除零，野指针问题导致线程奔溃，进程也会随之奔溃

线程是进程的执行分支，线程出现异常，就类似于进程出现异常，进而触发信号机制，终止进程，该进程内的所有线程也就随即退出
