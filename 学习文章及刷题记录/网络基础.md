...
ET：只会触发一次，一旦数据就绪，程序员应该尽可能一次把所有的就绪事件读完(需要程序员循环调用recv,通过返回值判断是否读取完毕)

LT：需不需要一次读取所有数据呢？不一定
...
#include<string>
class EventIten;
typedef int(*callback_t )(EventIntem *);//const&输入 *输出  &输入输出
class Epoller;
class EventIntem
{
    //当成一个结构体使用
    public:
    int sock;
    Epoller*R;//可以回指epoller
    
    //有关数据处理的回调函数，用来进行逻辑解耦的，
    //通信细节和数据处理模块使用该方法进行解耦
    callback_t recv_hadnler;
    callback_t send_handler;
    callback_t error_handler;
    std::string inbuffer;//读取到的数据缓冲区
    std::string outbuffer;//待发送的缓冲区
    
    public:
    EventIten():sock(0),R(nullptr),recv_handlder(nullptr),send_handler(nullptr),error_handler(nullptr)
    {
        
    }
    
    void MangerCallBack(callback_t _recv,callback_t _send,callback_t _err)
    {
        recv handler=recv;
        send_handler=_send;
        error_handler=_err;
...
class Epoller{
    ...
    std::unordered_map<int,EventItem>event_items;//将sock映射到EventItem中
    public:
    ...
    void Event(int sock,uint32_t event,const EventIten&item)
    {
        struct epoll_event ev;
        ev.events=0;
        ev.evemts|=event;
        ...
        {
            cerr<<"epoll error:"<<sock<<endl;
        }
        else
        {
     
            enent_items.insert(sock,item);
        }
    }
    ...
    void DelEvent(int sock)
    {
        ...
        {
            std::cerr<<"epoll_ctl_err fd::"<<fd<<endl;
        }
    ...
    void Dispatch(int timeout)
    {
        ...
        struct epoll_event revs[1000];
        for(;;)
        {
            //如果底层特定事件就绪，就把对应的事件分派给指定的回调函数进行统一处理
            int num=epoll_wait(epfd,revs,MAX_LEN,timeout);
            for(int i=0;i<num;i++)
            {
                int sock=revs[i].data.fd;
                if(revs[i].events&EPOLLIN)
                {//读事件就绪，执行度方法，如果读取回调被设置，就调用读回调
                ...
                {//写事件就绪
                    if(event_items[sock].sned_handler)
                    {
...
int main()
{
   ...
   //将我们的listen_sock托管给epoller epoller.AddEvent(listen_sock,EPOLLIN|EPOLLET,item);
    int timeout=1000;
    while(true)
    {
...
int accepter(EventItem*item)
{
    while(trur)
    {
        ...
        else
        {
            ...
            //epoll经常设置读事件就绪，而写事件我们使按需打开
            epoll->AddEvent(sock,EPOLL|EPOLLET,tmp);
...
int recver_helper(int sock,string*out)
{
    ...
    {
        char buffer[128]={0};
        ssize_t size=recv(sock,buffer,sizeof(buffer)-1,o);
        if(size<0)
        {
            if(errno==EAGAIN||errno==EWOULEBLOCK)
            {
                //循环读取完毕
                return 0;
            }
        ...
        {
            buffer[size]=0;
            *out+=buffer;//将我们读取到的得到的内容无脑添加到inbuffer中
            
        }
    }
}

void recver(EventIter*item)
{
    //需要整体读，非阻塞
    //根据发来的数据流，进行饱和包之间的分离，防止粘包问题
    //针对一个一个的报文协议反序列化
    //业务处理
    //形成响应报文，序列化转化为一个字符串encode
    //将数据写回
    if(recver(item->sock,&item->inbuffer)<0)
    {
        item->error handler;
        return -1;
    };
