[TOC]



# 动态规划刷题 

1.状态的定义

2.状态间的转移方程的定义

3.状态的初始化

4.返回结果 

### 基础问题

#### 1.斐波那契数列及其变形:本质都是斐波那契数列

青蛙跳台阶

矩形覆盖





#### 2.最大子数列的和

数组的最大连续和：

子问题：局部元素构成的数组的最大连续子数列的和

 F(i):  前i个元素组成的数组，它的最大连续子序列的和

F(i):F(i)=max(nums[i],F(i-1)+nums[i]);//以第i个元素结尾的元素的最大子序列的和

初始状态：F[0]=nums[0]

返回值:max(F(i))



#### 3.字符串分割

  单词是否可以成功分割

子问题：单词前几个字符组成的子串是否可以被成功分割(局部是否可以被分割)

状态F(i)：前i个字符是否可以被成功分割

转移方程：

F(i):j<i&&f(j)&&[j+1,i]

![image-20210905215718946](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20210905215718946.png)

初始状态:

F(0)&&[1,8]------->整体

F(0)=true;辅助状态

返回结果

整体能不能被分割

F(字符串长度):F(size());

```
vector<bool>canBreak(size()+1,0);
canBreak[0]=true;
for(int i=1;i<s.size();I++)
{
​	for(int j=0;j<i;j++)
	{
		if(canBreak[j]&&dift.find(s.substr(j,i)))

{

​		canBreak[i]=true;

​		break

}

}

}

return CanBeak[s.size()];


```

### 路径问题

#### [三角形_牛客题霸_牛客网 (nowcoder.com)](https://www.nowcoder.com/practice/2b7995aa4f7949d99674d975489cb7da?tpId)

> 描述
>
> 给出一个三角形，计算从三角形顶部到底部的最小路径和，每一步都可以移动到下面一行相邻的数字，

```cpp
例如，给出的三角形如下：

[[20],[30,40],[60,50,70],[40,10,80,30]]

最小的从顶部到底部的路径和是20 + 30 + 50 + 10 = 110。

注意：如果你能只用O（N）的额外的空间来完成这项工作的话，就可以得到附加分，其中N是三角形中的行总数。
```

> 最小路径和----三角矩阵
>
> 问题：最短路劲和
>
> 状态：从（0，0）到达（i，j)的最小路径和
>
> (i,j)------>(i+1,j),(i+1,j+1)
>
> 如果从下一行来推断上一行：
>
> (i,j)可能来自于(i-1,j)或者(i-1,j-1)
>
> (i-1,j),(i-1,j-1)-------->(i,j)
>
> F（i，j)=min(nums(i,j)+F(i-1,j),nums(i,j)+F(i-1,j-1))
>
> 每一行第一列和最后一列只有一条路径可以到达
>
> 第一列
>
> F（i，0）=F（i-1，0）+nums(i,0)
>
> 最后一列
>
> F(i,nums[i].size()-1)=nums(i,nums[i].size()-1)+F(i-1,nums[i-1].size()-1)
>
> 初始状态：
>
> F(0,0)=nums(0,0)

```cpp
//dp(i,j)表示从(0,0)点到(i,j)的最短路径
//而要到达(i,j)，就要先到达(i-1,j)或者(i-1,j-1)才行
//所以dp(i,j)=min{dp(i-1,j),dp(i-1,j-1)}
//而且要考虑第一列与最后一列的特殊情况
//要走第一列，只有一种可能性：dp[i][0]=dp[i-1][0]+triangle[i][j]
//每一行的最后一列：dp[i][j]=dp[i-1][j-1]+triangle[i][j]
class Solution {
public:
    int minimumTotal(vector<vector<int> > &triangle) {
        //判断是否符合条件
        if(triangle.empty())
            return 0;
        //构建dp数组
        vector<vector<int>>dp(triangle);
     /*   //处理第一列和最后一列
        for(int i=1;i<triangle.size();i++)
        {
            dp[i][0]=dp[i-1][0]+triangle[i][0];
            dp[i][i]=dp[i-1][i-1]+triangle[i][i];
        }
        
        for(int i=2;i<triangle.size();i++)
        {
            for(int j=1;j<triangle[i].size()-1;j++)
            {
                dp[i][j]=min(dp[i-1][j],dp[i-1][j-1])+triangle[i][j];
            }
        }
        int row=triangle.size()-1;
        int col=triangle[row].size();
        int Min=dp[row][0];
        for(int j=0;j<row;j++)
        {
            if(dp[row][j]<Min)
            {
                Min=dp[row][j];
            }
        }
        return Min;
        */
        //dp数组进行初始化，从第二行，第一列开始初始化
        for(int i=1;i<triangle.size();i++)
        {
            for(int j=0;j<=i;j++)
            {
                //如果是第一列特殊处理
                if(j==0)
                    dp[i][j]=dp[i-1][j]+triangle[i][j];
                //最后一列
                else if(j==i)
                    dp[i][j]=dp[i-1][j-1]+triangle[i][j];
                //普通情况
                else
                    dp[i][j]=min(dp[i-1][j],dp[i-1][j-1])+triangle[i][j];
            }
        }
        //选取最后一列的最小值
        int Min=dp[triangle.size()-1][0];
        for(int j=0;j<triangle.size();j++)
        {
            if(dp[triangle.size()-1][j]<Min)
                Min=dp[triangle.size()-1][j];
        }
        return Min;
        
    }
};
```

#### [求路径的个数_牛客题霸_牛客网 (nowcoder.com)](https://www.nowcoder.com/practice/166eaff8439d4cd898e3ba933fbc6358?tpId)

描述

一个机器人在m×n大小的地图的左上角（起点）。

机器人每次向下或向右移动。机器人要到达地图的右下角（终点）。

可以有多少种不同的路径从起点走到终点？

![image-20210919175056221](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20210919175056221.png)

```cpp
示例1
输入：2,1
返回值：1

示例2
输入：2,2
返回值：2
```

> 路径的个数:
>
> 从开始走到终点所走的路径个数
>
> 每次只能向下或者向右移动
>
> 问题：从（0，0）到最后一行最后一列的路径个数
>
> F(i,j):从（0，0）到（i,j）的路径个数
>
> （i-1，j）        (i,j-1)
>
> F(i,j)=F(i-1,j)+F(i,j-1)
>
> 初始状态：
>
> 使得：F(0,j)=1
>
> ​           F(i,0)=1;
>
> 最后返回F(row-1,col-1)
>



```cpp
class Solution {
public:
    /**
     * 
     * @param m int整型 
     * @param n int整型 
     * @return int整型
     */
    //题目中每次都可以从(i,j)向下或者向右走一次，即走到(i+1,j)或者走到(i,j+1)
    //定义状态F(i,j)表示从(0,0)到达(i,j)所有的路劲之和
    //而想要到达(i,j),可以从(i-1,j)或者(i,j-1)走到
    //因此F(i,j)=F(i-1,j)+F(i,j-1);
    //同时初始化第一行和第一列，因为到达第一行和第一列任意位置的路径个数只有一个，即直走
    
    int uniquePaths(int m, int n) {
       
        // write code here
        vector<vector<int>>dp(m,vector<int>(n,0));
        for(int i=0;i<m;i++)
        {
            dp[i][0]=1;
        }
        for(int j=1;j<n;j++)
        {
            dp[0][j]=1;
        }
        for(int i=1;i<m;i++)
        {
            for(int j=1;j<n;j++)
            {
                dp[i][j]=dp[i-1][j]+dp[i][j-1];
            }
        }
        return dp[m-1][n-1];
    }
};
```

#### [62. 不同路径](https://leetcode-cn.com/problems/unique-paths/)

> 一个机器人位于一个 m x n 网格的左上角 （起始点在下图中标记为 “Start” ）。
>
> 机器人每次只能向下或者向右移动一步。机器人试图达到网格的右下角（在下图中标记为 “Finish” ）。
>
> 问总共有多少条不同的路径？
>

![image-20210919181838403](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20210919181838403.png)

```
输入：m = 3, n = 7
输出：28
示例 2：

输入：m = 3, n = 2
输出：3
解释：
从左上角开始，总共有 3 条路径可以到达右下角。

1. 向右 -> 向下 -> 向下
2. 向下 -> 向下 -> 向右
3. 向下 -> 向右 -> 向下
   示例 3：

输入：m = 7, n = 3
输出：28
示例 4：

输入：m = 3, n = 3
输出：6
```

> 思路：这道题目与上面那个题目是相同的，都是求到达最后一点的所有路径

```cpp
class Solution {
public:
//dp(i,j)从(0,0)到(i,j)的路径为多少
//(i,j)----->(i,j+1)/(i+1,j)
//从(i-1,j)或者(i,j-1)可以走到i,j
//dp(i,j)=dp(i-1,j)+dp(i,j-1)
//初始化，使得第一行和第一列所有元素都为1
    int uniquePaths(int m, int n) {
        vector<vector<int>>dp(m,vector<int>(n,0));
        for(int j=0;j<n;j++)
        {
            dp[0][j]=1;
        }
        for(int i=0;i<m;i++)
        {
            dp[i][0]=1;
        }
        for(int i=1;i<m;i++)
        {
            for(int j=1;j<n;j++)
            {
                dp[i][j]=dp[i-1][j]+dp[i][j-1];
            }
        }
        return dp[m-1][n-1];

    }
};
```

#### [剑指 Offer II 098. 路径的数目](https://leetcode-cn.com/problems/2AoeFn/)

> 一个机器人位于一个 m x n 网格的左上角 （起始点在下图中标记为 “Start” ）。
>
> 机器人每次只能向下或者向右移动一步。机器人试图达到网格的右下角（在下图中标记为 “Finish” ）。
>
> 问总共有多少条不同的路径？
>

 

```cpp
示例 1：
输入：m = 3, n = 7
输出：28

示例 2：
输入：m = 3, n = 2
输出：3
解释：
从左上角开始，总共有 3 条路径可以到达右下角。
1. 向右 -> 向下 -> 向下
2. 向下 -> 向下 -> 向右
3. 向下 -> 向右 -> 向下

示例 3：
输入：m = 7, n = 3
输出：28

示例 4：
输入：m = 3, n = 3
输出：6
```

```cpp
//dp[i][j]:从(0,0)到达(i,j)的所有路径
//dp[i][j]=dp[i][j-1]+dp[i-1][j]
class Solution {
public:
    int uniquePaths(int m, int n) {
        vector<vector<int>>dp(m,vector<int>(n,0));
        for(int i=0;i<m;i++)
        dp[i][0]=1;
        for(int j=0;j<n;j++)
        dp[0][j]=1;
        for(int i=1;i<m;i++)
        {
            for(int j=1;j<n;j++)
            {
                dp[i][j]=dp[i-1][j]+dp[i][j-1];
            }
        }
        return dp[m-1][n-1];

    }
};
```



#### [63. 不同路径 II](https://leetcode-cn.com/problems/unique-paths-ii/)

> 一个机器人位于一个 m x n 网格的左上角 （起始点在下图中标记为“Start” ）。
>
> 机器人每次只能向下或者向右移动一步。机器人试图达到网格的右下角（在下图中标记为“Finish”）。
>
> 现在考虑网格中有障碍物。那么从左上角到右下角将会有多少条不同的路径？
>

![image-20210919183956176](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20210919183956176.png)

![image-20210919184015250](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20210919184015250.png)

```cpp
输入：obstacleGrid = [[0,0,0],[0,1,0],[0,0,0]]
输出：2
解释：
3x3 网格的正中间有一个障碍物。
从左上角到右下角一共有 2 条不同的路径：

1. 向右 -> 向右 -> 向下 -> 向下
2. 向下 -> 向下 -> 向右 -> 向右
```



![image-20210919184200119](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20210919184200119.png)



```cpp
输入：obstacleGrid = [[0,1],[0,0]]
输出：1
```

> 有限制条件求路径个数
>
> F(i,j)从（0，0）到（i,j）的路径个数
>
> F(i,j):
>
> ​		if(ob[i][j]==1)
>
> ​			f(i,j)=0;
>
> ​		else
>
> ​		F(i,j)=F(i.j-1)+F(i,j-1)
>
> 初始状态：
>
> 判断:
>
> 第一行：
>
> while(ob(i,0)==0)
>
> F(0,i)=1;
>
> 第一列：
>
> while(On(0,j)==0)
>
> F(j,0)=1;
>
> 二维矩阵的初始化：
>
> vector<vector<int>>PathNum(row,vector<int>(col,0))
>

```cpp
//这道题目比上面求路径和的题目加了一些限制，增加了障碍物，其实并不会影响我们整体的思路，整体思路还是和刚才那道题一样
//dp[i][j]:从(0,0)到达(i,j)所需要的所有路径总和
//dp[i][j]=dp[i-1][j]+dp[i][j-1]
//特殊处理：
//第一行：遇到石头前，到达前面这些点所走路径都为1，到达石头以及后面所走路径都是0，因为石头访问不了，那么石头后面的也就访问不了
//第一列：同理可得
//在中间位置遇到石头，dp[i][j]==0


class Solution {
public:
    int uniquePathsWithObstacles(vector<vector<int>>& obstacleGrid) {
        int row=obstacleGrid.size();//记录行数
        int col=obstacleGrid[0].size();//记录列数
        vector<vector<int>>dp(row,vector<int>(col,0));//定义dp数组
        //初始化第一列
        for(int i=0;i<row;i++)
        {
            if(obstacleGrid[i][0]==0)
            dp[i][0]=1;
            else
            break;
        }
        //初始化第一行
        for(int j=0;j<col;j++)
        {
            if(obstacleGrid[0][j]==0)
            dp[0][j]=1;
            else
            break;
        }
        //循环遍历dp数组
        for(int i=1;i<row;i++)
        {
            for(int j=1;j<col;j++)
            {
                if(obstacleGrid[i][j]==0)
                {
                    dp[i][j]=dp[i-1][j]+dp[i][j-1];
                }
                else
                {
                    dp[i][j]=0;
                }
            }
        }
        //返回最后一个结果
        return dp[row-1][col-1];


    }
};
```

#### [带权值的最小路径和_牛客题霸_牛客网 (nowcoder.com)](https://www.nowcoder.com/practice/23462ed010024fcabb7dbd3df57c715e?tpId)

> 描述
>
> 给定一个由非负整数填充的m x n的二维数组，现在要从二维数组的左上角走到右下角，请找出路径上的所有数字之和最小的路径。
> 注意：你每次只能向下或向右移动。

```cpp
示例1
输入：
[[1,2],[5,6],[1,1]]
返回值：
8
```

> 带权值得最小路径和
>
> 最小路劲和：
>
> 从二维数组左上角走到右下角，求说有数字路径最小之和
>
> F(i,j):从（0，0）到(i,j)的最小路径和
>
> (i-1,j),(i,j-1)这两个位置走到(i,j)
>
> F(i,j)=min(F(i-1,j),F(i,j-1))+nums[i]
>
> 初始化：
>
> 将第一行，第一列进行初始化
>
> F(0,0)=nums(0,0)
>
> F(0,1)=F(0,0)+num(0,1)
>
> 即第一行的地推公式为：F(0,j)=F(0,j-1)+nums(0,j)//J从1开始
>
> 第一列递推公式：F(i,0)=F(i-1,0)+nums(i,0);//i从1开始
>

```cpp
class Solution {
public:
    /**
     * 
     * @param grid int整型vector<vector<>> 
     * @return int整型
     */
    //问题：与上面那道题几乎一样，上面那道题是求从左上角到右下角的所有路径之和
    //这道题是求从左上角到右下角的最短路径
    //F(i,j)代表从(0,0)到(i,j)的最短路
    //走到(i,j)前的上一步是(i-1,j)或者(i,j-1)
    //所以F(i,j)=min{F(i-1,j),F(i,j-1)}+nums(i,j);
    //同时初始化第一行和第一列全部变为1；
    int minPathSum(vector<vector<int> >& grid) {
        // write code here
        int row=grid.size();
        int col=grid[0].size();
        vector<vector<int>>dp(row,vector<int>(col,0));
        dp[0][0]=grid[0][0];
        for(int i=1;i<row;i++)
        {
            dp[i][0]=dp[i-1][0]+grid[i][0];
        }
        
        for(int j=0;j<col;j++)
        {
            dp[0][j]=dp[0][j-1]+grid[0][j];
        }
        for(int i=1;i<row;i++)
        {
            for(int j=1;j<col;j++)
            {
                dp[i][j]=min(dp[i-1][j],dp[i][j-1])+grid[i][j];
            }
        }
        return dp[row-1][col-1];

     }
};
```

### 背包问题：

> 有n个物品和一个大小为m得背包，给定数组A代表每个物品得大小和数组V代表每个物品得价值，问最多能装入背包的总价值是多大
>
> 问题：从n个商品中做选择，当包的大小为m时得最大价值
>
> 状态：F(i,j):从i个商品中做选择，当包得大小价值为j时的最大价值
>
> A[i-1]<=j:
>
> F(i,j)=max{F(i-1,j),F(i-1,j-A[i-1])+V[i-1]}
>
> A[i-1]>j:
>
> F(i,j)=F(i-1,j)
>
> 新增的第i个商品，如果选择放入第i个商品：
>
> 1.不需要取出已有的商品：
>
> 2.需要取出已有的部分商品
>
> ​          a.第i个商品的价值小于取出的商品价值总和（不放入）
>
> ​           b.第·i个商品的价值大于去除的商品价值总和
>
> 5个物品，最大大小为4

![image-20210919205320548](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20210919205320548.png)(ps:来源于代码随想录)



```cpp
dp[i][j]:
[0,i]物品任取，然后放进容量为j的背包里
dp[i][j]
不放物品i，dp[i-1][j]
放物品i:dp[i-1][j-weight[i]]+value[i]
 dp[i][j]=max(dp[i-1][j],dp[i-1][j-weight[i]]+value[i]) 

```

#### 二维dp数组

```cpp
01背包问题是最经典的动态规划问题：
即一个背包最多能装m物品，价值为value[i]的物品重量为weight[i],如何装才能使背包展现最大价值
问题转化：
dp[i][j]:背包容量为j在[0,i]件物品中选择价值最大的物品
对于第i件物品，有两种情况，选与不选
选择第i件物品：dp[i][j]=max(dp[i-1][j],dp[i][d-weight[i]]+value[i])
不选第i件物品：dp[i][j]=dp[i-1][j]
对于第i件物品的选与不选，主要取决于i与weight[i]的关系
j>weight[i]:dp[i][j]=max(dp[i-1][j],dp[i][d-weight[i]]+value[i])
j<wieght[i]:dp[i][j]=dp[i-1][j]
初始化问题：
1.dp数组的构建：vector<vector<int>>dp(weight.size()+1,vector<int>(bagweight+1,0))
2.dp数组的初始化：倒叙初始化
for(int j=bagweight;j>=0;j--)
   dp[0][j]=dp[0][j-weight[0]]+value[0];
3.遍历物品，不论先便利哪个都可以，在这里我们先遍历物品，在遍历背包
初始时第一行已经初始化了，所以让i=1,j=0;
for(int i=1;i<weight.size();i++)
{
		
#include<iostream>
using namespace std;
#include<vector>
void test()
{
	vector<int>weight = { 1,3,4 };
	vector<int>value = { 15,20,30 };
	int bagweight = 4;
	vector<vector<int>>dp(weight.size()+1 , vector<int>(bagweight + 1, 0));
	//初始化背包
	for (int j = bagweight; j >= weight[0]; j--)
	{
		dp[0][j] = dp[0][j - weight[0]] + value[0];
	}
	//双层遍历背包
	//先便利物品，在遍历背包
	for (int i = 1; i<weight.size(); i++)
	{
		for (int j = 0; j <= bagweight; j++)
		{
			if (j < weight[i])
			{
				dp[i][j] = dp[i - 1][j];
			}
			else
			{
				dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - weight[i]] + value[i]);
			}
		}
	}
	cout << dp[weight.size() - 1][bagweight] << endl;
}
int main()
{

	test();
	return 0;
}
```

#### 一维dp数组

```
dp[j]=max(dp[j],dp[j-weight[i]]+value[i])

初始化：

dp[j]=0
```

```cpp
//一维滚动数组实现01背包问题
#include<iostream>
#include<vector>
; using namespace std;
void test()
{
	vector<int>weight = { 1,3,4 };
	vector<int>value = { 15,20,30 };
	int bagweight = 4;
	vector<int>dp(bagweight + 1, 0);
	for (int i = 0; i<weight.size(); i++)
	{
		for (int j = bagweight; j >= weight[i]; j--)
		{

			dp[j] = max(dp[j], dp[j - weight[i]] + value[i]);
		}
	}
	cout << dp[bagweight] << endl;
}
int main()
{
	test();
	return 0;
}
```

字符串的分割：

分割回文串，使得每一部分都是回文串的最小分割次数

最大分割次数一定：即size-1

问题：字符串s的最小分割次数

状态F(i)：字符串前面i个字符所需要的最小分割次数

转移方程：F(i)=

aab:

F(1):"a"  0

F(2):"aa" 0

F(3):"aab"  F(2)+1

//F(i):j<i F(i),[1,i]----->分割成两部分----->[1,j]

F(i):j<i&&[j+1,i]是回文串:min(F(j)+1)

初始状态：

F(1,i)是回文串：F(i)=0

初始状态：F(1)=0

 F(i)最大分割次数：F(i)=i-1;

### 最长公共子序列

#### 最长公共子串

找出两个字符串的最长公共子串，注意子串和子序列的区别，子串指的是连续的序列，子序列是可以不连续，但是相对顺序不变的序列

这道题目采取动态规划的方法，我们创建一个二维表格，将数组的长度当作表格的行和列，例如：字符串str1=acbcbcef，str2=abcbced,那么由观察可知最长公共子序列为bcbce,长度为5，我们创建如下表格，如果两个值相同，我们将表格中的值改为1，不相同改为0

|      | a    | b    | c    | b    | c    | e    | d    |
| ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| a    | 1    | 0    | 0    | 0    | 0    | 0    | 0    |
| c    | 0    | 0    | 1    | 0    | 1    | 0    | 0    |
| b    | 0    | 1    | 0    | 1    | 0    | 0    | 0    |
| c    | 0    | 0    | 1    | 0    | 1    | 0    | 0    |
| b    | 0    | 1    | 0    | 1    | 0    | 0    | 0    |
| c    | 0    | 0    | 1    | 0    | 1    | 0    | 0    |
| e    | 0    | 0    | 0    | 0    | 0    | 1    | 0    |
| f    | 0    | 0    | 0    | 0    | 0    | 0    | 0    |

![image-20211102093234007](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20211102093234007.png)

由上面的表格我们可以看到，一共由5个最长公共子序列，其中最长的一个为5，同时如果是公共子序列，他们位于同一条对角线上，

我们将表格数据该成如下所示d：

```=d
dp[i][j]dp[i-1][j-1]+1;
```

![image-20211102093706739](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20211102093706739.png)

由表格观察可知：

当str1[i]!=str2[j]时，dp(i)(j)=0;

当str1[i]==str2[j]:

if(i==0||j==0):dp[i] [j]=1

else:dp[i] [j]=dp[i-1] [j-1] +1

初始化：dp[i] [j]默认全部初始化为0，等到str1[i]==str2[j]时在进行操作

代码如下所示：

```cpp
#include<iostream>
#include<vector>
#include<string.h>
using namespace std;
int main()
{
	string str1;
	string str2;
	cin >> str1;
	cin >> str2;
	int ret = 0;
	vector<vector<int>>dp(str1.size(), vector<int>(str2.size(), 0));
	for (int i = 0; i < str1.size(); i++)
	{
		for (int j = 0; j < str2.size(); j++)
		{
			if (i == 0 && str1[i] == str2[j])
			{
				dp[i][j] = 1;
			}
			else if (j == 0 && str1[i] == str2[j])
			{
				dp[i][j] = 1;
			}
			else if (j > 0 && i > 0 && str1[i] == str2[j])
			{
				dp[i][j] = dp[i - 1][j - 1] + 1;
			}
			ret = max(ret, dp[i][j]);

		}

	}
	cout << ret << endl;
	return 0;
}
```

![image-20211102095056669](https://raw.githubusercontent.com/qingyan520/Cloud_img/master/img/image-20211102095056669.png)

#### 最长公共子序列

给定两个字符串strA[]=“ abfcab",strB[]=" abcfbc"，求最长公共子序列。

建立如下表格进行观察：

|      |      | a    | b    | c    | f    | b    | c    |
| ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |
|      | 0    | 0    | 0    | 0    | 0    | 0    | 0    |
| a    | 0    | 1    | 1    | 1    | 1    | 1    | 1    |
| b    | 0    | 1    | 2    | 2    | 2    | 2    | 2    |
| f    | 0    | 1    | 2    | 2    | 3    | 3    | 3    |
| c    | 0    | 1    | 2    | 3    | 3    | 3    | 4    |
| a    | 0    | 1    | 2    | 3    | 3    | 3    | 4    |
| b    | 0    | 1    | 2    | 3    | 3    | 4    | 4    |

通过观察我们可以知道，if(strA[i]==strB[j])  dp[i,j]=dp[i-1,j-1]+1;else  dp[i,j]=max(dp[i-1,j],dp[i,j-1])

代码如下所示：

```cpp
class Solution {
public:
    int longestCommonSubsequence(string text1, string text2) {
        //与上面表格相对应，创建dp数组
        vector<vector<int>>dp(text1.size()+1,vector<int>(text2.size()+1,0));
        for(int i=1;i<=text1.size();i++)
        {
            for(int j=1;j<=text2.size();j++)
            {
                if(text1[i-1]==text2[j-1])
                {
                    dp[i][j]=dp[i-1][j-1]+1;
                }
                else
                {
                    dp[i][j]=max(dp[i-1][j],dp[i][j-1]);
                }
            }
        }
        return dp[text1.size()][text2.size()];

    }
};
```

### 最长递增子序列

#### [300. 最长递增子序列](https://leetcode-cn.com/problems/longest-increasing-subsequence/)

> 给你一个整数数组 nums ，找到其中最长严格递增子序列的长度。
>
> 子序列是由数组派生而来的序列，删除（或不删除）数组中的元素而不改变其余元素的顺序。例如，[3,6,2,7] 是数组 [0,3,1,6,2,2,7] 的子序列。
>

 

```cpp
示例 1：
输入：nums = [10,9,2,5,3,7,101,18]
输出：4
解释：最长递增子序列是 [2,3,7,101]，因此长度为 4 。
    
示例 2：
输入：nums = [0,1,0,3,2,3]
输出：4
    
示例 3：
输入：nums = [7,7,7,7,7,7,7]
输出：1
```

本题目求的是最长递增子序列，我们创建dp数组，dp中的i代表以nums[i]结尾的最长递增子序列

我们以第一个nums=[10,9,2,5,3,7,101,18]为例进行说明：

dp[0]代表以nums[0] (10)结尾的最长公共子序列dp[0]=1

dp[1]代表以nums[1] (9)，dp[1]=1

dp[2]代表以nums[2] (2)，dp[2]=1

dp[3]代表以nums[3] (5)，dp[3]=dp[2]+1=2

dp[4]代表以nums[4] (7)，dp[4]=dp[3]+1=3

dp[5]代表以nums[5] (101)，dp[5]=max(dp[0]……dp[4])+1=4

dp[6]代表以nums[6] (18)，dp[6]=max(dp[0]…….dp[4])+1=4

由上面我们可以知道最长递增子序列为4，同时我们也可以得到递推公式：

```cpp
//定义i为当前位置，j为小于i的位置
dp[i]=1;
if(nums[j]<nums[i])
{
    dp[i]=max(dp[i],dp[j]+1);
}
ret=max(ret,dp[i]);
```

本题目代码如下：

```cpp
class Solution {
public:
    int lengthOfLIS(vector<int>& nums) {
        vector<int>dp(nums.size(),0);
        //dp[i]代表以nums[i]结尾的最长公共子序列
        int ret=-1;
        for(int i=0;i<nums.size();i++)
        {
            dp[i]=1;
            for(int j=0;j<i;j++)
            {
                if(nums[j]<nums[i])
                {
                    dp[i]=max(dp[i],dp[j]+1);
                }
            }
            ret=max(ret,dp[i]);
        }
        return ret;
    }
};
```



### 兑换

#### [322. 零钱兑换](https://leetcode-cn.com/problems/coin-change/)

给你一个整数数组 coins ，表示不同面额的硬币；以及一个整数 amount ，表示总金额。

计算并返回可以凑成总金额所需的 最少的硬币个数 。如果没有任何一种硬币组合能组成总金额，返回 -1 。

你可以认为每种硬币的数量是无限的。

```cpp
示例 1：
输入：coins = [1, 2, 5], amount = 11
输出：3 
解释：11 = 5 + 5 + 1
    
示例 2：
输入：coins = [2], amount = 3
输出：-1
    
示例 3：
输入：coins = [1], amount = 0
输出：0
    
示例 4：
输入：coins = [1], amount = 1
输出：1

示例 5：
输入：coins = [1], amount = 2
输出：2
```

创建dp数组，dp[i]代表兑换i所需要的最少零钱数量

我们以coins=[1,2,5],amount=11为例子进行说明

当我们选1块钱时dp[11]=1+dp[11-1],也就是说换一张1块钱，然后再看后面dp[10]所需要的最小张数

dp[11]=1+dp[11-2],先换一张2块钱，再看后面9元所需要的最小张数

dp[11]=1+dp[11-5],先换一张5块钱，然后看剩下6块钱所需要的最小张数

综上所述dp[11]=min{1+dp[10],1+dp[9],1+d[6]}

我们利用下面表格进行分析

| dp[i] | 最终取值 | 1            | 2            | 5            |
| ----- | -------- | ------------ | ------------ | ------------ |
| 0     | 0        | ×            | ×            | ×            |
| 1     | 1        | 1+dp[1-1]=1  | ×            | ×            |
| 2     | 1        | 1+dp[2-1]=2  | 1+dp[2-2]=1  | ×            |
| 3     | 2        | 1+dp[3-1]=2  | 1+dp[3-2]=2  | ×            |
| 4     | 2        | 1+dp[4-1]=3  | 1+dp[4-2]=2  | ×            |
| 5     | 1        | 1+dp[5-1]=3  | 1+dp[5-2]=3  | 1+dp[5-5]=1  |
| 6     | 2        | 1+dp[6-1]=2  | 1+dp[6-2]=3  | 1+dp[6-5]=2  |
| 7     | 2        | 1+dp[7-1]=3  | 1+dp[7-2]=2  | 1+dp[7-5]=2  |
| 8     | 3        | 1+dp[8-1]=3  | 1+dp[8-2]=3  | 1+dp[8-5]=3  |
| 9     | 3        | 1+dp[9-1]=4  | 1+dp[9-2]=3  | 1+dp[9-5]=3  |
| 10    | 2        | 1+dp[10-1]=4 | 1+dp[10-2]=4 | 1+dp[10-5]=2 |
| 11    | 3        | 1+dp[11-1]=3 | 1+dp[11-2]=4 | 1+dp[11-5]=3 |

错误的答案，自己写的，但是感觉比较好理解，就先贴在这里了

```cpp
class Solution {
public:
    int coinChange(vector<int>& coins, int amount) {
        vector<int>dp(amount+1,INT_MAX/2);
        dp[0]=0;
       // dp[1]=1;
        //dp[i]=min(dp[i-coins[i]]+1

        for(int i=1;i<=amount;i++)
        {
            //对照表格，找到每一次取零钱是所对应的最小值
           int ret=INT_MAX/2;
            for(int j=0;j<coins.size();j++)
            {
                if(i>=coins[j])
                ret=min(ret,dp[i-coins[j]]+1);
            }
            dp[i]=ret;
        }
        if(dp[amount]==INT_MAX/2)
        return -1;
        return dp[amount];
    }
};
```

完整代码如下：

```cpp
class Solution {
public:
    int coinChange(vector<int>& coins, int amount) {
        vector<int>dp(amount+1,INT_MAX/2);
        dp[0]=0;
       // dp[1]=1;
        //dp[i]=min(dp[i-coins[i]]+1

        for(int i=1;i<=amount;i++)
        {
           
            for(int j=0;j<coins.size();j++)
            {
                if(i>=coins[j])
                dp[i]=min(dp[i],dp[i-coins[j]]+1);
            }
        }
        return dp[amount]==INT_MAX/2? -1:dp[amount];
    }
};
```

### 错排问题

#### [发邮件](https://www.nowcoder.com/questionTerminal/95e35e7f6ad34821bc2958e37c08918b)

> NowCoder每天要给很多人发邮件。有一天他发现发错了邮件，把发给A的邮件发给了B，把发给B的邮件发给了A。于是他就思考，要给n个人发邮件，在每个人仅收到1封邮件的情况下，有多少种情况是所有人都收到了错误的邮件？
>  即没有人收到属于自己的邮件。

思路：

> 这道题目是经典的错排题目，思路如下： 
>
> 用A、B、C……表示写着ｎ位友人名字的信封，a、b、c……表示ｎ份相应的写好的信纸。把错装的总数为记作D(n)。假设把**ａ错装进Ｂ里了**，包含着这个错误的一切错装法分两类：  
>
> ​    (1)b装入Ａ里，这时每种错装的其余部分都与A、B、a、b无关，应有D(n－2)种错装法。  
>
> ​    (2)ｂ装入A、B之外的一个信封，这时的装信工作实际是把（除a之外的）n－1份信纸ｂ、ｃ……装入（除B以外的）n－1个信封A、C……，显然这时装错的方法有D(n－1)种。  
>
> ​    总之在ａ装入B的错误之下，共有错装法D(n－2)＋D(n－1)种。  
>
> ​    a装入C，装入D……的n－2种错误之下，同样都有D(n－1)＋D(n－2)种错装法，因此D(n)＝(n－1)[D(n－1)＋D(n－2)]



代码：

```cpp
#include<iostream>
#include<vector>
using namespace std;
int main()
{
    while(cin>>n)
    {
        vector<long long int>dp(n+1,0);
        dp[0]=0;
        dp[1]=0;
        
        for(int i=2;i<=n;i++)
        {
            dp[i]=(i-1)*(dp[i-1]+dp[i-2]);
        }
        cout<<dp[n]<<endl;
    }
}

```

