...
```cpp
//这个题目有了思路之后代码还是有点难写，我们该如何判断这个节点位于左子树还是右子树呢，这里我们先默认让两个节点往左子树去找，定义pInleft,qInleft记录结果，那么如果两个节点都在左子树那么(pInleft&&qInleft)就为true,我们需要向左子树继续寻找，如果(!pInleft&&!qInleft)为true，代表两个节点都在右子树，向右子树寻找，那么剩下的情况就是两个节点在左右两个子树中，返回结果
class Solution {
public:
